...
exports.createUser = (req, res) => {
  const {firstname,lastname,email,username,password,admin} =req.body;
  const hash = bcrypt.hashSync(password, 10);
  console.log(hash)
    return User.create( {firstname:firstname,lastname:lastname,email:email,username:username,password:hash,admin:admin})
      .then((createdUser) => {
